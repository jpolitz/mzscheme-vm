...
Mail = function(dest, msg) {
    this.dest = dest;
    this.msg = msg;
};

Mail.makeInstance = function(dest, msg) {
    return new Mail(dest, msg);
};

Mail.prototype.toWrittenString = function(cache) {
    return ("(mail " + toWrittenString(this.dest) + ", " +
            toWrittenString(this.msg) + ")");
}

Mail.prototype.toString = Mail.prototype.toWrittenString;

Mail.prototype.toDisplayedString =  Mail.prototype.toWrittenString;

Mail.prototype.toDomNode = function(cache) {
    //    cache.put(this, true);
    var node = document.createElement("span");
    node.appendChild(document.createTextNode(this.toWrittenString()));
    return node;
};
...
Parcel = function(ws, serverMsgs, mails) {
    this.ws = ws;
    this.serverMsgs = serverMsgs
    this.mails = mails;
};

Parcel.makeInstance = function(ws, serverMsgs, mails) {
    return new Parcel(ws, serverMsgs, mails);
};

Parcel.prototype.toWrittenString = function(cache) {
    return ("(parcel " + toWrittenString(this.ws) + ", " +
            toWrittenString(this.serverMsgs) + ", " +
            toWrittenString(this.mails) + ")");
}

Parcel.prototype.toString = Parcel.prototype.toWrittenString;

Parcel.prototype.toDisplayedString =  Parcel.prototype.toWrittenString;

Parcel.prototype.toDomNode = function(cache) {
    //    cache.put(this, true);
    var node = document.createElement("span");
    node.appendChild(document.createTextNode(this.toWrittenString()));
    return node;
...
types.isMail = function(x) { return x instanceof Mail; };
types.isParcel = function(x) { return x instanceof Parcel; };
...
types.mail = Mail.makeInstance;
types.parcel = Parcel.makeInstance;
