...
//	};
	...
	var wrapJsValue = function(x) {
        ...
        var wrapSexpCheck = function (sexp, check) {
            if(sexp instanceof Array) {
                var result = types.EMPTY;
                for(var i = sexp.length-1; i >= 0; i--) {
                    result = Cons.makeInstance(wrapSexpCheck(sexp[i], check), 
                                               result);
                }
                return result;
            }
                ...
                throw "Invalid s-exp in wrapSexpCheck --- the JavaScript value " + 
                    String(sexp) + 
                    " didn't pass the provided predicate";
            }
        };

        var wrapSexp = function (sexp) {
            return wrapSexpCheck(sexp, function(_) { return true; });
        };

        var unwrapSexpCheck = function (sexp, check) {
            var arr = [];
            if (types.isPair(sexp)) {
                arr.push(unwrapSexpCheck(sexp.first(), check));
                return arr.concat(unwrapSexpCheck(sexp.rest(), check));
            }
                ...
                throw "Invalid s-exp in unwrapSexpCheck --- the Racket value " + 
                    String(sexp) + 
                    " didn't pass the provided predicate";
            }
        };

        var unwrapSexp = function(sexp) {
            return unwrapSexpCheck(sexp, function(_) { return true; });
        };
...
//	helpers.remove = remove;
	...
	helpers.getKeyCodeName = getKeyCodeName;
        ...
        helpers.unwrapSexpCheck = unwrapSexpCheck;
        helpers.wrapSexpCheck = wrapSexpCheck;

        helpers.wrapSexp = wrapSexp;
        helpers.unwrapSexp = unwrapSexp;
